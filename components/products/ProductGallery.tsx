...
export default function ProductGallery({
    ...
    return (
        <div className={`flex flex-col-reverse ${!noThumbnail ? "lg:flex-row" : ""} gap-4 lg:items-start lg:sticky lg:top-24 h-fit`}>
            {/* Thumbnails Wrapper */}
            {!noThumbnail && (
                    ...
                    >
                        {galleryImages.map((img, index) => (
                            <button
                                ...
                                className={`relative shrink-0 aspect-square w-20 lg:w-full rounded-md overflow-hidden border-2 ${activeIndex === index ? "border-primary" : "border-transparent"
                                    }`}
                            >
                                <Image
                                    src={img}
